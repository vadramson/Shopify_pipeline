...
with DAG('Run_Shopify_Streamlit_ELT_Pandas', default_args=default_args, schedule_interval=None, catchup=False) as dag:
    ...
    run_streamlit_analytics = DockerOperator(
        ...
        api_version='auto',
        auto_remove=True,  
        command="streamlit run /app/streamlit_analytics.py",  # streamlit run /app/streamlit_analytics.py --server.port 8590  Specify the command to run streamlit_analytics.py
