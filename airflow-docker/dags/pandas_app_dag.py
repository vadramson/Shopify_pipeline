...
with DAG('Run_Shopify_ELT_Pandas', default_args=default_args, schedule_interval=None) as dag:
    # Task to trigger pandas-app using DockerOperator
    run_pandas_app = DockerOperator(
        privileged=True,
        task_id='run_pandas_app_task',
        image='shopify_pipeline-pandas-app',  # shopify_pipeline-pandas-app:latest This should match your container name or image name
        container_name='vadrama_pandas',  # The container name for the pandas app
        api_version='auto',
        auto_remove=True,  # Automatically remove the container after task completion
        #command="python /app/main.py",  # Modify based on how the pandas-app runs
